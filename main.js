...
var Modals = /*#__PURE__*/function () {
  ...
  _createClass(Modals, null, [{
    key: "showError",
    value: function showError(text, input) {
      ...
      popoverDiv.style.top = "".concat(input.getBoundingClientRect().height + 8, "px");
      popoverDiv.style.left = "".concat(input.getBoundingClientRect().width / 2 - popoverDiv.getBoundingClientRect().width / 2, "px");
      arrowDiv.style.left = "".concat(popoverDiv.getBoundingClientRect().width / 2 - arrowDiv.getBoundingClientRect().width + 3, "px");
...
function TicketsMove_typeof(obj) { "@babel/helpers - typeof"; return TicketsMove_typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (obj) { return typeof obj; } : function (obj) { return obj && "function" == typeof Symbol && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }, TicketsMove_typeof(obj); }
...
function TicketsMove_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
function TicketsMove_defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, TicketsMove_toPropertyKey(descriptor.key), descriptor); } }
function TicketsMove_createClass(Constructor, protoProps, staticProps) { if (protoProps) TicketsMove_defineProperties(Constructor.prototype, protoProps); if (staticProps) TicketsMove_defineProperties(Constructor, staticProps); Object.defineProperty(Constructor, "prototype", { writable: false }); return Constructor; }
function TicketsMove_toPropertyKey(arg) { var key = TicketsMove_toPrimitive(arg, "string"); return TicketsMove_typeof(key) === "symbol" ? key : String(key); }
function TicketsMove_toPrimitive(input, hint) { if (TicketsMove_typeof(input) !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (TicketsMove_typeof(res) !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); }
/* eslint-disable max-len */
var TicketsMove = /*#__PURE__*/function () {
  function TicketsMove() {
    TicketsMove_classCallCheck(this, TicketsMove);
    this.clearVars();
  }
  TicketsMove_createClass(TicketsMove, [{
    key: "clearVars",
    ...
    key: "emptyGhostElement",
    value: function emptyGhostElement(e) {
      var elemBelow = document.elementFromPoint(e.clientX, e.clientY);
      var closestColumnItems = elemBelow.closest('.column-items');
    ...
    key: "ticketGrab",
    value: function ticketGrab(e, item) {
      e.preventDefault();
      if (!item) return;
      this.draggedEl = item;
      ...
      this.ghostElEmpty.style.height = "".concat(this.draggedElCoords.height - 16, "px");
      this.ghostElEmpty.style.backgroundColor = '#d5dbde';
      document.body.append(this.ghostEl);
      this.ghostEl.style.left = "".concat(this.draggedElCoords.left - 8, "px");
      this.ghostEl.style.top = "".concat(this.draggedElCoords.top - 8, "px");
      this.ghostEl.style.width = "".concat(this.draggedElCoords.width, "px");
    ...
    key: "ticketMove",
    value: function ticketMove(e) {
      e.preventDefault();
      if (!this.draggedEl) return;
      this.ghostEl.style.left = "".concat(e.pageX - this.cursX, "px");
      this.ghostEl.style.top = "".concat(e.pageY - this.cursY, "px");
      this.emptyGhostElement(e);
      e.target.classList.add('grabbing');
    }
  }, {
    key: "ticketLeave",
    value: function ticketLeave() {
    ...
    key: "grabbingRemove",
    value: function grabbingRemove() {
      var deleteGrabbing = Array.from(document.querySelectorAll('.grabbing'));
      if (deleteGrabbing) deleteGrabbing.forEach(function (tag) {
        return tag.classList.remove('grabbing');
      });
    }
  }]);
  return TicketsMove;
...
function Tickets_typeof(obj) { "@babel/helpers - typeof"; return Tickets_typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (obj) { return typeof obj; } : function (obj) { return obj && "function" == typeof Symbol && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }, Tickets_typeof(obj); }
...
function Tickets_toPropertyKey(arg) { var key = Tickets_toPrimitive(arg, "string"); return Tickets_typeof(key) === "symbol" ? key : String(key); }
function Tickets_toPrimitive(input, hint) { if (Tickets_typeof(input) !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (Tickets_typeof(res) !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); }
/* eslint-disable max-len */
...
var Tickets = /*#__PURE__*/function () {
  ...
  Tickets_createClass(Tickets, [{
    ...
    value: function init() {
      ...
      var sortArr = this.ticketsArr.slice();
      this.lastTicketId = sortArr.sort(function (a, b) {
        return a.id - b.id;
      }).at(-1).id;
      this.events();
      this.updateList();
    ...
    value: function events() {
      var _this = this;
      this.tasks.addEventListener('pointerdown', function (e) {
        return _this.clickEvents(e);
      });
      this.tasks.addEventListener('pointermove', function (e) {
        return _this.ticketsMove.ticketMove(e);
      });
      this.tasks.addEventListener('pointerleave', function () {
        if (_this.ticketsMove.ticketLeave()) {
          _this.ticketsMove.ticketLeave();
          _this.updateList();
        }
      });
      this.tasks.addEventListener('pointerup', function () {
        var draggedEl = _this.ticketsMove.ticketDrop();
        if (!draggedEl) return;
        _this.insertTicket(draggedEl);
      });
    }
  }, {
    key: "clickEvents",
    ...
    key: "addAnotherTicket",
    value: function addAnotherTicket(e) {
      var _this2 = this;
      var colItems = e.target.closest('.column').querySelector('.column-items');
      ...
      colItems.after(form);
      var addAnotherCard = e.target.closest('.add-item');
      ...
      form.addEventListener('submit', function (evt) {
        ...
        _this2.addCard(colItems.dataset.id, form.ticketFormValue.value);
      });
      setTimeout(function () {
        return form.ticketFormValue.focus();
      }, 10);
      form.ticketFormValue.addEventListener('focus', function () {
        return Modals.hideError();
      });
      form.ticketFormValue.addEventListener('keydown', function (event) {
        if (event.code === 'Enter') {
    ...
    key: "addCard",
    value: function addCard(column, text) {
      this.lastTicketId += 1;
    ...
    value: function deleteCard(e) {
      var deleteItemId = +e.target.closest('.column-item').dataset.id;
      var deleteItemIndex = this.ticketsArr.findIndex(function (item) {
    ...
    key: "insertTicket",
    value: function insertTicket(draggedEl) {
      var draggedElRes = draggedEl.draggedElDiv;
      var insertItemRes = draggedEl.insertItemObj;
      var travelerTicket = this.ticketsArr.find(function (ticket) {
        return ticket.id === +draggedElRes.dataset.id;
      });
      ...
      var travelerTicketIndex = this.ticketsArr.findIndex(function (ticket) {
        return ticket.id === +draggedElRes.dataset.id;
      });
      ...
      if (!insertItemRes.id && !insertItemRes.position) {
        ...
        var insertTicketIndex = this.ticketsArr.findIndex(function (ticket) {
          return ticket.id === +insertItemRes.id;
        });
    ...
    value: function updateList() {
      var _this3 = this;
      Array.from(this.columnItems).forEach(function (column) {
        var col = column;
        col.innerHTML = '';
      });
      this.ticketsArr.forEach(function (ticket) {
        var col = Array.from(_this3.columnItems).find(function (column) {
          return ticket.column === column.dataset.id;
        });
        var ticketDiv = document.createElement('div');
        ticketDiv.className = 'column-item';
        ticketDiv.dataset.id = ticket.id;
        ticketDiv.innerHTML = "\n                <div class=\"column-item__text\">".concat(ticket.text, "</div>\n                <div class=\"column-item__delete\"></div>           \n            ");
        col.append(ticketDiv);
      });
      localStorage.setItem('cards', JSON.stringify(this.ticketsArr));
    }
  }], [{
    key: "addAnotherTicketCancel",
    value: function addAnotherTicketCancel() {
      var form = document.querySelector('.ticket-form');
      if (!form) return;
      form.remove();
      var addAnotherCardButtonHidden = document.querySelector('.add-item.hidden');
...
var tickets = new Tickets(demoContent);
tickets.init();
